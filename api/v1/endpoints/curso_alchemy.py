...
from models.curso_model_alchemy import CursoModelAlchemy
...
async def post_curso(curso: CursoSchema, db: AsyncSession = Depends(get_session)):
    #SQL Alchemy
    novo_curso = CursoModelAlchemy(titulo=curso.titulo,
...
async def get_cursos(db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(CursoModelAlchemy)
        result = await session.execute(query)
        cursos: List[CursoModelAlchemy] = result.scalars().all()
        print(" V1")
        return cursos
...
async def get_curso(curso_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(CursoModelAlchemy).filter(CursoModelAlchemy.id == curso_id)
        result = await session.execute(query)
